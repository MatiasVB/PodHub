...
export const PodcastDetailPage: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
              ...
              <div key={season}>
                ...
                <div className="space-y-3">
                  {episodes.map((episode: Episode) => (
                    <div key={episode.id} className="block p-4 bg-gray-50 rounded-lg">
                      <div className="flex items-start gap-4">
                        <span className="text-2xl font-bold text-gray-400">
                          {episode.number}
                        </span>

                        <Link to={`/episodes/${episode.id}`} className="flex-1 min-w-0 group">
                        ...
                        {/* Creator Actions - Only show if isCreator */}
                        {isCreator && (
      ...
      {/* Edit Episode Modal */}
      {showEditEpisodeModal && selectedEpisode && (
        <EditEpisodeModal
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            ...
            <p className="text-gray-600 mb-6">
              ¿Estás seguro que deseas eliminar el episodio <strong>"{episodeToDelete.title}"</strong>?
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button
                onClick={() => deleteEpisodeMutation.mutate(episodeToDelete.id)}
                disabled={deleteEpisodeMutation.isPending}
                className="px-6 py-2 bg-red-600 text-white rounded-lg font-medium hover:bg-red-700 transition-colors disabled:opacity-50"
              >
                {deleteEpisodeMutation.isPending ? 'Eliminando...' : 'Eliminar Definitivamente'}
              </button>
            </div>
            ...
            {deleteEpisodeMutation.isError && (
              <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                <p className="text-sm text-red-600">
                  Error al eliminar el episodio. Por favor, intenta de nuevo.
                </p>
              </div>
            )}
          </div>
